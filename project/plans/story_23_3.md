...
**Test results:** 623 tests passing (up from 687 pre-deletion: 65 old tests removed with `test_btcmesh_server.py`, 0 skipped - the 10 previously-`@unittest.skip`'d tests were dead scaffolding entirely within that file, testing an old destination-format validation path that no longer exists by design).
...
- **A real bug was found and fixed via the second round of hardware testing** (see below): `_broadcast()` crashed with an unhandled `AttributeError` when `rpc_client` is `None`, instead of failing gracefully - a concrete instance of the exact pattern Issue 18 describes. Fixed by checking for `rpc_client is None` up front and sending an immediate, informative `BTC_NACK` instead of attempting the call. Added a regression test (`test_broadcast_with_no_rpc_client_sends_nack_and_fires_callback`) and updated Issue 18 with this finding.

**Manual verification against real hardware - completed:**
- **First pass**, with one Meshtastic device available: Meshtastic connection confirmed (`python btcmesh_server_cli.py -p /dev/cu.usbserial-0001` connected to node `!7c5b4418`, logging exactly the expected sequence). RPC-failure tolerance confirmed for real (not just in mocks) - Tor/the RPC node was unreachable in this environment, and the server correctly logged the error and continued anyway straight through to `Server started...`, exactly matching the designed behavior. Full chunk-transfer and a live Ctrl+C were deferred at this point - only one working device was reachable (a second candidate port hit the "wedged, needs a physical power cycle" state from Issue 12/16, and a third turned out to be the same physical device already in use).
- **Second pass**, after the devices were power-cycled: both were reachable (`!aee5ab3c` and `!7c5b4418`). Sent a real 15-chunk transaction end-to-end - every chunk ACKed cleanly on the first attempt (no RF retries needed), and the full narrative appeared exactly as designed in both the server's terminal output and `logs/btcmesh_server.log`: received chunk → requesting next chunk → ... → all chunks received, reassembly successful → broadcasting transaction to Bitcoin network. This run is what surfaced the `_broadcast()`/`rpc_client=None` crash described above (RPC was still unreachable in this environment). After fixing it, re-ran the exact same transaction: the server now sent a clean `BTC_NACK|<session>|Bitcoin RPC not connected`, and the client failed in ~2 seconds with `"Transaction failed: Bitcoin RPC not connected"` instead of the previous ~2-minute silent timeout ending in an uninformative `"No final ACK from relay"`.
- **Ctrl+C shutdown**: confirmed correct via automated test (`test_keyboard_interrupt_disconnects_and_returns_0`, which injects a real `KeyboardInterrupt` exception the same way Python's own SIGINT handler does). A live OS-level signal test was not achievable in this session's tool environment - `kill -INT <pid>` sent to a backgrounded process (from a separate invocation, and even from within the same shell invocation) never reached it; a trivial, unrelated `while True: time.sleep(1)` script reproduced the identical non-response, confirming this is a sandboxing limitation rather than anything specific to this story's code. A real Ctrl+C typed directly into an interactive terminal (not achievable from this tool-calling environment) would be the only way to close out this specific sub-item, but the shutdown logic itself is verified correct.
...
| `server/receiver.py` | `_broadcast()` now handles `rpc_client=None` gracefully (bug fix found via hardware testing) |
...
| `tests/test_server_receiver.py` | +3 tests (2 gap closure, 1 bug-fix regression) |
| `tests/test_btcmesh_server.py` | Deleted (1905 lines) |
| `project/issues.txt` | +Issue 18, later updated with the real-hardware finding and fix |
