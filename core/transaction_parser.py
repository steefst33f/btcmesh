...
def read_varint(data: bytes, offset: int) -> (int, int):
    ...
    return 0,0 # Should not be reached if prefixes are valid, but as a fallback for linter.
...
def decode_raw_transaction_hex(hex_str: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ValueError(f"Failed to decode raw transaction due to parsing error (IndexError): {e}. TX length: {len(data)}, Offset: {offset}")
