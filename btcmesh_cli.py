        ...
        if 'decoded' in packet and packet['decoded']['portnum'] == 'TEXT_MESSAGE_APP':
            ...
            if sender_node_id == _expected_sender_node_id:
                ...
                if len(msg_parts) > 1 and msg_parts[1] == _current_session_id:
                    cli_logger.info(f"CLI received relevant message for session {_current_session_id} from {sender_node_id}: '{message_text}'")
                    _message_queue.put(message_text)
                elif len(msg_parts) > 1 and msg_parts[1] != _current_session_id:
                    cli_logger.debug(f"CLI received message for different session ID. Expected: '{_current_session_id}', Got: '{msg_parts[1]}'. Msg: '{message_text}'")
                else: # Message for our session but not parsable for session ID, or from wrong sender
                    cli_logger.debug(f"CLI received message from {sender_node_id} but not for current session ID or malformed for session check. Msg: '{message_text}'")
            elif sender_node_id != _expected_sender_node_id:
                 # Log if it's a text message but not from the expected sender
                cli_logger.debug(f"CLI received text message from unexpected sender {sender_node_id} (expected {_expected_sender_node_id}). Msg: '{message_text}'")

    except UnicodeDecodeError:
        cli_logger.warning(f"CLI could not decode payload: {packet['decoded']['payload']}")
    except Exception as e:
        cli_logger.error(f"CLI error in on_receive_text_message_cli: {e}", exc_info=True)
...
def cli_main(args=None, injected_iface=None, injected_logger=None, injected_message_receiver=None):
    ...
    if iface is None:
        ...
        (injected_logger or cli_logger).error("Failed to initialize Meshtastic interface in cli_main.")
        raise RuntimeError('Failed to initialize Meshtastic interface')

    _current_session_id = getattr(args, 'session_id', None) or generate_session_id() # Set global session ID
    ...
    if use_injected_receiver and not args.dry_run:
        message_iterator = injected_message_receiver(timeout=ACK_TIMEOUT, session_id=_current_session_id)
        ...
        logger.info(f"CLI Subscribed to Meshtastic messages for session {_current_session_id}. Expecting replies from {args.destination}")
        ...
        if not args.dry_run:
            logger.error("CLI reached an unexpected state: not a dry run, but no message receiver configured.")
            return 1 # Error exit
    ...
    if not args.dry_run:
        ...
        try:
            while i < total_chunks:
                ...
                while current_retries < MAX_RETRIES:
                    ...
                    try:
                        ...
                        print(f"Sent chunk {chunk_num}/{total_chunks} for session {_current_session_id} to {args.destination}")
                        logger.info(f"Sent chunk {chunk_num}/{total_chunks} for session {_current_session_id} to {args.destination}")
                    except Exception as e:
                    ...
                    try:
                        if use_injected_receiver:
                            ...
                            received_msg_text = _message_queue.get(block=True, timeout=ACK_TIMEOUT)
                        ...
                        logger.warning(f"Message generator exhausted for session {_current_session_id} while waiting for ACK for chunk {chunk_num}/{total_chunks}.")
                        # This implies a timeout or premature end from the generator
                        current_retries += 1
                        if current_retries < MAX_RETRIES:
                            print(f"Retrying chunk {chunk_num}/{total_chunks} (attempt {current_retries + 1} of {MAX_RETRIES}) due to timeout/generator end.")
                            logger.warning(f"Retrying chunk {chunk_num}/{total_chunks} (attempt {current_retries + 1} of {MAX_RETRIES}) for session {_current_session_id} due to timeout/generator end.")
                        ...
                        logger.warning(f"Timeout waiting for ACK/NACK for chunk {chunk_num}/{total_chunks} in session {_current_session_id}.")
                        current_retries += 1
                        if current_retries < MAX_RETRIES:
                            print(f"Retrying chunk {chunk_num}/{total_chunks} (attempt {current_retries + 1} of {MAX_RETRIES}) due to timeout.")
                            logger.warning(f"Retrying chunk {chunk_num}/{total_chunks} (attempt {current_retries + 1} of {MAX_RETRIES}) for session {_current_session_id} due to timeout.")
                    ...
                    msg_session_id_from_payload = msg_parts[1] if len(msg_parts) > 1 else ""

                    # Already filtered by onReceive for session ID if using queue, but double check for injected
                    if use_injected_receiver and msg_session_id_from_payload != _current_session_id:
                        logger.warning(f"CLI received message for wrong session ID. Expected '{_current_session_id}', got '{msg_session_id_from_payload}'. Msg: '{received_msg_text}'")
                        ...
                        if current_retries < MAX_RETRIES:
                            print(f"Retrying chunk {chunk_num}/{total_chunks} (attempt {current_retries + 1} of {MAX_RETRIES}) due to unexpected message for other session.")
                            logger.warning(f"Retrying chunk {chunk_num}/{total_chunks} (attempt {current_retries + 1} of {MAX_RETRIES}) for session {_current_session_id} due to unexpected message for other session: {received_msg_text}")
                    ...
                    if msg_type == "BTC_CHUNK_ACK" and len(msg_parts) >= 5:
                        try:
                            ...
                            if msg_chunk_num_int == chunk_num and msg_status == "OK":
                                ...
                                logger.info(f"ACK for chunk {chunk_num}/{total_chunks} in session {_current_session_id}. Server command: {msg_command}")
                                if msg_command == "REQUEST_CHUNK" and len(msg_parts) >= 6:
                                    ...
                                    if next_chunk_req.isdigit() and int(next_chunk_req) == chunk_num + 1:
                                        ...
                                        logger.warning(f"Malformed REQUEST_CHUNK in ACK for chunk {chunk_num}: '{received_msg_text}'")
                                        # Treat as unrecognized, fall through to retry
                                elif msg_command == "ALL_CHUNKS_RECEIVED":
                                    print(f"All transaction chunks ACKed by server for session {_current_session_id}.")
                                    logger.info(f"All transaction chunks ACKed by server for session {_current_session_id}.")
                                    ...
                                    print(f"Waiting for final transaction broadcast confirmation from server for session {_current_session_id}...")
                                    logger.info(f"Waiting for final TX ACK/NACK for session {_current_session_id}")
                                    final_ack_retries = 0
                                    final_tx_confirmed = False
                                    while final_ack_retries < MAX_RETRIES: # Use a similar retry for final confirmation
                                        try:
                                            if use_injected_receiver:
                                                final_msg_text = next(message_iterator)
                                            else:
                                                final_msg_text = _message_queue.get(block=True, timeout=ACK_TIMEOUT * 2) # Longer timeout for broadcast
                                            ...
                                            if final_msg_session == _current_session_id:
                                                if final_msg_type == "BTC_ACK" and len(final_msg_parts) >= 4 and final_msg_parts[2] == "SUCCESS":
                                                    txid = final_msg_parts[3].split("TXID:",1)[-1] if "TXID:" in final_msg_parts[3] else "UNKNOWN_TXID"
                                                    print(f"Transaction successfully broadcast by relay. TXID: {txid}")
                                                    logger.info(f"Received final BTC_ACK for session {_current_session_id}: {final_msg_text}")
                                                    final_tx_confirmed = True
                                                    break # from final_ack_retries loop
                                            elif final_msg_type == "BTC_NACK" and len(final_msg_parts) >= 3 and final_msg_parts[2] == "ERROR":
                                                error_details = "|".join(final_msg_parts[3:])
                                                print(f"Relay reported an error broadcasting transaction: {error_details}")
                                                logger.error(f"Received final BTC_NACK for session {_current_session_id}: {error_details}")
                                                ...
                                                logger.warning(f"Received unexpected final message for session {_current_session_id}: {final_msg_text}")
                                                # Fall through to retry if not MAX_RETRIES for final ack
                                        except (StopIteration, queue.Empty): # Timeout for final ACK/NACK
                                            logger.warning(f"Timeout waiting for final TX ACK/NACK for session {_current_session_id} (attempt {final_ack_retries + 1})")
                                        final_ack_retries += 1
                                        if final_ack_retries < MAX_RETRIES and not final_tx_confirmed:
                                            print(f"Retrying wait for final confirmation (attempt {final_ack_retries + 1})")
                                            time.sleep(RETRY_TIMEOUT)

                                    if not final_tx_confirmed:
                                        print(f"No final transaction confirmation received from relay for session {_current_session_id}.")
                                        logger.error(f"Timeout or max retries for final TX confirmation for session {_current_session_id}.")
                                        raise SystemExit(4) # Specific exit code for no final confirmation
                                ...
                                logger.warning(f"Received ACK for wrong chunk/status in session {_current_session_id}. Expected chunk {chunk_num}, Got: '{received_msg_text}'")
                                # Treat as unrecognized, fall through
                        except (ValueError, IndexError) as e:
                            logger.warning(f"Malformed BTC_CHUNK_ACK received in session {_current_session_id}: '{received_msg_text}', Error: {e}")
                            # Treat as unrecognized, fall through

                    # Handle BTC_NACK
                    elif msg_type == "BTC_NACK" and len(msg_parts) >= 4: # Check session already done by onReceive or earlier logic
                        try:
                            ...
                            error_detail = "|".join(msg_parts[2:]) # Grab all parts after session_id as detail.
                            print(f"Received NACK for session {_current_session_id} (applies to chunk {chunk_num}): {error_detail}")
                            logger.warning(f"Received NACK for session {_current_session_id} (chunk {chunk_num}): {error_detail}. Message: {received_msg_text}")
                            
                            current_retries += 1
                            if current_retries < MAX_RETRIES:
                                print(f"Retrying chunk {chunk_num}/{total_chunks} (attempt {current_retries + 1} of {MAX_RETRIES}) due to NACK.")
                                logger.warning(f"Retrying chunk {chunk_num}/{total_chunks} (attempt {current_retries + 1} of {MAX_RETRIES}) for session {_current_session_id} due to NACK.")
                                time.sleep(RETRY_TIMEOUT) # Wait a bit before retrying
                                continue # Retry sending the same chunk
                            else:
                            ...
                            logger.warning(f"Malformed BTC_NACK received in session {_current_session_id}: '{received_msg_text}', Error: {e}")
                    ...
                    # Handle BTC_SESSION_ABORT from server
                        ...
                        abort_reason = "|".join(msg_parts[2:]) if len(msg_parts) > 2 else "Unknown reason"
                        print(f"Session {_current_session_id} aborted by server: {abort_reason}")
                        logger.error(f"Session {_current_session_id} aborted by server: {abort_reason}. Message: {received_msg_text}")
                        raise SystemExit(2) # Abort session
                    ...
                    logger.warning(f"Unrecognized or unexpected response for chunk {chunk_num}/{total_chunks} in session {_current_session_id}: '{received_msg_text}'")
                    print(f"Unrecognized response from server: {received_msg_text}")
                    current_retries += 1
                    if current_retries < MAX_RETRIES:
                        print(f"Retrying chunk {chunk_num}/{total_chunks} (attempt {current_retries + 1} of {MAX_RETRIES}) due to unrecognized response.")
                        logger.warning(f"Retrying chunk {chunk_num}/{total_chunks} (attempt {current_retries + 1} of {MAX_RETRIES}) for session {_current_session_id} due to unrecognized response.")
                        time.sleep(RETRY_TIMEOUT) # Wait a bit before retrying
                        continue # Retry sending the same chunk
                    else:
            ...
            if i >= total_chunks: # Should be i == total_chunks if successful
                 logger.info(f"All chunks processed and final TX confirmed for session {_current_session_id}.")
                 print(f"Transaction transmission completed for session {_current_session_id}.")
                 return 0 # Success!
            else: # Should not be reached if logic is correct, implies loop exited unexpectedly
                logger.error(f"Exited chunk processing loop prematurely for session {_current_session_id}. Chunks processed: {i}/{total_chunks}")
            ...
            logger.info(f"SystemExit caught in cli_main for session {_current_session_id} with code {e.code}")
            return e.code
        except Exception as e:
            logger.error(f"Unhandled exception during CLI message processing for session {_current_session_id}: {e}", exc_info=True)
            print(f"An unexpected error occurred: {e}")
            return 1 # Generic error
        finally:
            if not use_injected_receiver and not args.dry_run: # Cleanup for pubsub
                logger.info(f"CLI Unsubscribing from Meshtastic messages for session {_current_session_id}.")
                try:
                    ...
                    logger.error(f"Error during pub.unsubscribe for session {_current_session_id}: {e_unsub}", exc_info=True)
            if iface and hasattr(iface, 'close') and not injected_iface : # Close iface if we opened it
                logger.info(f"CLI closing Meshtastic interface for session {_current_session_id}.")
                iface.close()
            # Reset globals for potential re-entry (e.g. in tests, though not typical for CLI)
            _message_queue = None
            _current_session_id = None
            _expected_sender_node_id = None
            logger.info(f"CLI main execution finished for session (was {_current_session_id}).")
    ...
    if not args.dry_run:
        logger.error("Execution reached an unexpected point after main processing loop without returning a status.")
